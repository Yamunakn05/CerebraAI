...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&duration=4000&pause=1000&color=38BDF8&center=true&vCenter=true&width=750&lines=CerebraAI;Advanced+Deep+Learning+Diagnostics;Brain+Tumor+MRI+Analysis;Multi-Role+Clinical+Workspace+Platform" alt="CerebraAI Typing SVG" />
...
| Endpoint | Method | Description | Access / Auth |
|----------|:------:|-------------|:-------------:|
| `/api/auth/register` | `POST` | Register new user account (Patient, Doctor, etc.) | 🔓 **Public (No Auth)** |
| `/api/auth/login` | `POST` | Authenticate user via Email/Username & Password | 🔓 **Public (No Auth)** |
| `/api/auth/logout` | `POST` | Terminate session & clear server cache | 🔒 **Login Required** |
| `/api/auth/me` | `GET` | Retrieve authenticated user profile | 🔒 **Login Required** |
| `/api/upload` | `POST` | Upload & run deep learning analysis on MRI | 🔒 **Login Required** |
| `/api/analysis/latest` | `GET` | Fetch user's latest MRI diagnostic result | 🔒 **Login Required** |
| `/api/analysis/<id>` | `GET` | Retrieve specific diagnostic scan by ID | 🔒 **Login Required** |
| `/api/scans/<id>/image` | `GET` | Stream original high-res MRI from GridFS | 🔒 **Login Required** |
| `/api/scans/<id>/gradcam` | `GET` | Generate & stream Grad-CAM 2D overlay | 🔒 **Login Required** |
| `/api/dashboard` | `GET` | Fetch role-adaptive dashboard metrics | 🔒 **Login Required** |
| `/api/chatbot` | `POST` | Query CerebraBot LLM medical assistant | 🔒 **Login Required** |
| `/api/appointments/book` | `POST` | Schedule patient doctor consultation | 🔒 **Login Required** |
| `/api/appointments/update` | `POST` | Doctor accepts / rejects consultation | 🔒 **Login Required** |
| `/api/scans/status` | `POST` | Update physician scan review status | 🔒 **Login Required** |
| `/api/reports/pdf/<id>` | `GET` | Generate & download PDF report with QR | 🔒 **Login Required** |
| `/api/reports/audio/<id>` | `GET` | Synthesize & stream gTTS audio report | 🔒 **Login Required** |
| `/api/medicine/search` | `GET` | Query pharmaceutical intelligence database | 🔒 **Login Required** |
| `/api/emergency` | `GET` | Fetch 24/7 emergency dispatch directory | 🔓 **Public (No Auth)** |
